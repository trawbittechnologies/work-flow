...
> = {
  ...
  "This Month": [
    { day: "W1", completed: 20, inProgress: 15 },
    { day: "W2", completed: 45, inProgress: 25 },
    { day: "W3", completed: 72, inProgress: 35 },
    { day: "W4", completed: 95, inProgress: 42 },
  ],
  "This Quarter": [
    { day: "M1", completed: 30, inProgress: 20 },
    { day: "M2", completed: 65, inProgress: 35 },
    { day: "M3", completed: 98, inProgress: 45 },
  ],
};
...
export function ProjectProgressChart() {
  ...
  return (
    <div className="bg-white border border-[#EAEDF2] rounded-2xl p-4 sm:p-6 shadow-2xs relative">
      {/* Header */}
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-[#4B5563] bg-white border border-[#E5E7EB] px-2.5 sm:px-3 py-1.5 rounded-xl hover:bg-[#F9FAFB] active:scale-95 transition-all cursor-pointer shadow-2xs"
          >
            <span>{timeframe}</span>
            <ChevronDown className="h-3.5 w-3.5 text-[#9CA3AF]" />
          </button>
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-32 bg-white border border-[#E5E7EB] rounded-xl shadow-lg z-20 py-1 text-xs font-medium animate-in">
              {["This Week", "This Month", "This Quarter"].map((t) => (
                <button
                  key={t}
                  onClick={() => handleSelect(t)}
                  className="w-full text-left px-3 py-1.5 hover:bg-[#F3F4F6] text-[#374151] flex items-center justify-between cursor-pointer"
                >
                  <span>{t}</span>
                  {timeframe === t && (
      ...
      {/* Legend */}
      <div className="flex items-center gap-4 sm:gap-6 mb-4 sm:mb-6 text-xs font-medium text-[#4B5563]">
        <div className="flex items-center gap-2">
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Line
              ...
              stroke="#88C315"
              strokeWidth={2.5}
              dot={{ fill: "#88C315", r: 3.5, strokeWidth: 2, stroke: "#FFFFFF" }}
              activeDot={{ r: 5.5, fill: "#88C315" }}
            />
            <Line
              type="monotone"
              dataKey="inProgress"
              stroke="#9CA3AF"
              strokeWidth={2}
              dot={{ fill: "#9CA3AF", r: 3, strokeWidth: 2, stroke: "#FFFFFF" }}
