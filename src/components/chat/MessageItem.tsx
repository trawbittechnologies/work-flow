    ...
    <div className={cn("flex flex-col mb-4 group relative", isOwn ? "items-end" : "items-start")}>
      ...
      <div className={cn("relative flex items-center gap-2 max-w-[85%] md:max-w-[75%]", isOwn ? "flex-row-reverse" : "flex-row")}>
        ...
        >
          {/* Reply Context Bar */}
          {message.replyTo && (
            <div className={cn("p-2 rounded-xl mb-2 text-xs border-l-2", isOwn ? "bg-white/10 border-[#C3D946]" : "bg-surface-alt border-[#0A1237]")}>
              ...
              {att.mimeType?.startsWith("image/") ? (
                ...
                <a
                  ...
                  className={cn(
                    ...
                    isOwn ? "bg-white/10 hover:bg-white/20 border-white/10" : "bg-surface-alt hover:bg-surface-alt/80 border-border"
                  )}
                >
                  <FileText className={cn("h-5 w-5", isOwn ? "text-[#C3D946]" : "text-[#0A1237]")} />
            ...
            <div className="flex flex-wrap gap-1 mt-2 pt-1 border-t border-border-subtle/40">
              {Object.entries(reactions).map(([emoji, count]) => (
                <button
                  ...
                  className="px-2 py-0.5 rounded-full bg-surface-alt/80 border border-border text-[11px] font-bold flex items-center gap-1 hover:scale-105 transition-transform"
                >
                  <span>{emoji}</span>
          ...
          <div className={cn("flex items-center justify-end gap-1 mt-1 text-[10px] font-bold", isOwn ? "text-[#C3D946]" : "text-text-muted")}>
        ...
        {/* Hover Action Bar (Slack / WhatsApp style) */}
        <div
          className={cn(
        ...
        >
          ...
          {quickEmojis.slice(0, 3).map((emoji) => (
            <button
              key={emoji}
              onClick={() => handleToggleReaction(emoji)}
              className="p-1 hover:bg-surface-alt rounded-lg text-xs transition-transform active:scale-125 cursor-pointer"
            >
              {emoji}
            </button>
          ))}
          ...
          <button
            ...
            className="p-1 text-text-muted hover:text-[#0A1237] dark:hover:text-[#C3D946] rounded-lg hover:bg-surface-alt transition-colors cursor-pointer"
            title="Add reaction"
          >
          ...
          {isOwn && (
            <button
              ...
              className="p-1 text-text-muted hover:text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors cursor-pointer"
            ...
            <div className="absolute top-full mt-1 bg-surface border border-border rounded-xl p-2 shadow-xl flex gap-1.5 z-30">
              {quickEmojis.map((emoji) => (
                <button
                  key={emoji}
                  onClick={() => handleToggleReaction(emoji)}
                  className="p-1.5 hover:bg-surface-alt rounded-lg text-sm hover:scale-125 transition-transform cursor-pointer"
                >
                  {emoji}
                </button>
              ))}
