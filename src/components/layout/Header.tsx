...
export function Header({ user, unreadNotifications = 0 }: HeaderProps) {
  ...
  return (
    <header className="h-[var(--header-height)] fixed top-0 right-0 left-0 md:left-[var(--sidebar-width)] z-20 bg-surface/85 backdrop-blur-xl border-b border-border flex items-center px-4 md:px-6 gap-3 transition-all duration-200 ease-in-out">
      {/* Mobile logo */}
      <Link href="/dashboard" className="flex items-center gap-2 md:hidden flex-shrink-0">
        <div className="h-8 w-8 bg-gradient-to-tr from-indigo-600 to-violet-500 rounded-xl flex items-center justify-center shadow-sm text-white">
      ...
      <form onSubmit={handleSearch} className="flex-1 max-w-md">
        <div className="relative group">
          ...
          <input
            ...
            className={cn(
              "w-full h-9 pl-9 pr-12 text-xs rounded-xl border",
              "bg-surface-alt/60 text-text-primary shadow-xs",
        ...
        <Link href="/notifications">
          <Button variant="ghost" size="icon" className="relative text-text-secondary hover:text-text-primary hover:bg-surface-alt" aria-label="Notifications">
            ...
            {unreadNotifications > 0 && (
              <span className="absolute top-1.5 right-1.5 h-2 w-2 bg-indigo-600 border border-surface rounded-full animate-pulse" />
            )}
          </Button>
        </Link>
            ...
            <div className="absolute right-0 top-full mt-2 w-60 bg-surface border border-border rounded-2xl shadow-xl overflow-hidden z-50 scale-in">
              ...
              <div className="p-1.5 space-y-0.5">
                <Link
                  ...
                  className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-text-secondary hover:bg-surface-alt hover:text-text-primary transition-colors"
                >
              ...
              <div className="p-1.5 border-t border-border bg-surface-alt/20">
                <button
                  onClick={() => signOut({ callbackUrl: "/login" })}
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-text-secondary hover:bg-danger-subtle hover:text-danger transition-colors cursor-pointer"
                >
